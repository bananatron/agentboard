...
func TestSuggestionCRUD(t *testing.T) {
	...
	task, _ := database.CreateTask(ctx, projectID, "Suggest Target", "")

	sug, err := database.CreateSuggestion(ctx, projectID, task.ID, db.SuggestionEnrichment, "claude", "Better title", "Rename to X")
	if err != nil {
	...
	// Get by ID
	got, err := database.GetSuggestion(ctx, projectID, sug.ID)
	if err != nil {
...
func TestListPendingSuggestions(t *testing.T) {
	...
	task, _ := database.CreateTask(ctx, projectID, "Sug Task", "")

	database.CreateSuggestion(ctx, projectID, task.ID, db.SuggestionEnrichment, "claude", "S1", "msg1")
	database.CreateSuggestion(ctx, projectID, task.ID, db.SuggestionProposal, "claude", "S2", "msg2")

	pending, err := database.ListPendingSuggestions(ctx, projectID)
	if err != nil {
...
func TestUpdateSuggestionStatus(t *testing.T) {
	...
	task, _ := database.CreateTask(ctx, projectID, "Sug Task", "")
	sug, _ := database.CreateSuggestion(ctx, projectID, task.ID, db.SuggestionHint, "claude", "Hint", "A hint")

	if err := database.UpdateSuggestionStatus(ctx, projectID, sug.ID, db.SuggestionAccepted); err != nil {
		t.Fatalf("updating status: %v", err)
	}

	got, _ := database.GetSuggestion(ctx, projectID, sug.ID)
	if got.Status != db.SuggestionAccepted {
		t.Errorf("status: got %q, want %q", got.Status, db.SuggestionAccepted)
	}

	// Should no longer appear in pending list
	pending, _ := database.ListPendingSuggestions(ctx, projectID)
	if len(pending) != 0 {
...
func TestListSuggestionsByTask(t *testing.T) {
	...
	t1, _ := database.CreateTask(ctx, projectID, "Task 1", "")
	t2, _ := database.CreateTask(ctx, projectID, "Task 2", "")

	database.CreateSuggestion(ctx, projectID, t1.ID, db.SuggestionEnrichment, "claude", "S1", "for t1")
	database.CreateSuggestion(ctx, projectID, t2.ID, db.SuggestionProposal, "claude", "S2", "for t2")
	database.CreateSuggestion(ctx, projectID, t1.ID, db.SuggestionHint, "claude", "S3", "also for t1")

	sugs, err := database.ListSuggestionsByTask(ctx, projectID, t1.ID)
	if err != nil {
	...
	sugs2, _ := database.ListSuggestionsByTask(ctx, projectID, t2.ID)
	if len(sugs2) != 1 {
...
func TestListSuggestionsByStatus(t *testing.T) {
	...
	task, _ := database.CreateTask(ctx, projectID, "Filter Task", "")
	s1, _ := database.CreateSuggestion(ctx, projectID, task.ID, db.SuggestionEnrichment, "claude", "S1", "msg")
	database.CreateSuggestion(ctx, projectID, task.ID, db.SuggestionProposal, "claude", "S2", "msg")

	// Dismiss s1
	database.UpdateSuggestionStatus(ctx, projectID, s1.ID, db.SuggestionDismissed)

	dismissed, err := database.ListSuggestions(ctx, projectID, db.SuggestionDismissed)
	if err != nil {
	...
	pending, _ := database.ListSuggestions(ctx, projectID, db.SuggestionPending)
	if len(pending) != 1 {
...
func TestSuggestionsCascadeOnTaskDelete(t *testing.T) {
	...
	task, _ := database.CreateTask(ctx, projectID, "Cascade Task", "")
	database.CreateSuggestion(ctx, projectID, task.ID, db.SuggestionEnrichment, "claude", "S1", "msg")
